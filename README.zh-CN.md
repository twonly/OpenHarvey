...
- **由你掌控的个人记忆**：在 Labs 开启后，让 Agent 记住长期工作偏好，也可以自己添加、编辑、删除或关闭。当前指令优先；合同事实与依据仍来自本次材料。
- **和 Agent 审改同一份合同**：圈选条款提出修改要求，直接保存原生待审修订。修订、批注与正文联动定位；接受文字修改和解决批注分别处理。
- **定位清楚，历史可追溯**：工作稿目录双向联动、自动保存、连续保存记录分组、命名重要版本、恢复历史为新版本；保留上传原件，下载修订版或清洁版。

DOCX 审改可由**正式账号在设置 → Labs 自行开启或关闭**，默认关闭；关闭保留已保存的修订、批注和版本。使用 SuperDoc 1.46.3，采用 AGPL 路线。首版为单人编辑；PDF 保留阅读、分析和建议能力。清洁版需要先处理全部待审修订；复杂排版保真及完整 Word/WPS 接受、拒绝兼容性仍在验证。

[查看新增功能](https://openharvey.com/features#latest) · [配置与能力边界](docs/memory-and-review.md)
