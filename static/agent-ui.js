...
export const toolLabels={websearch:'搜索网络',webfetch:'读取网页',read:'读取原文',grep:'搜索材料',glob:'查找文件',skill:'加载 Skill',write:'写入文件',edit:'编辑文件',bash:'处理文件',todowrite:'更新任务清单',question:'等待答复'};
export function toolHTML(p,stopped,open){
  ...
  const label={pending:'等待',running:'执行中',completed:'完成',error:'失败',interrupted:'已中断'}[status]||status;
  const glyph={grep:'search',glob:'search',edit:'write',todowrite:'check',question:'skill'}[p.tool]||p.tool;
  const detail=s.detail||(status==='completed'?(p.tool==='question'?'已收到你的答复':'操作已完成'):status==='error'||status==='interrupted'?'操作未完成':'等待执行结果');
  const action=p.tool==='question'&&status==='completed'?'收到答复':toolLabels[p.tool]||p.tool;
...
export function executionPhase(state){
  if(state.permissions?.length)return {kind:'permission',label:'等待你的许可'};
  if(state.questions?.length)return {kind:'question',label:'等待你的答复'};
  if(state.status?.type==='retry')return {kind:'retry',label:'等待重试',detail:state.status.message||'模型服务暂时不可用'};
  if(pendingRequest(state))return {kind:'pending',label:'正在准备'};
  ...
  if(running)return {kind:'tool',label:toolLabels[running.tool]||'正在处理',hasText};
  const last=turn.at(-1);
  if(last&&!last.info?.time?.completed&&!last.info?.finish&&last.parts?.some(p=>p.type==='text'&&p.text?.trim()))return {kind:'generating',label:'正在生成',hasText};
  return {kind:'processing',label:'正在处理',hasText};
}
export function thinkingHTML(state){
  const phase=executionPhase(state);
  if(phase.kind!=='retry'&&(phase.kind!=='processing'||phase.hasText))return '';
...
export function todosHTML(state,expanded){
  ...
  return `<details class="bui-tasks" ${expanded??active?'open':''}><summary>${icon('check')}<b>任务清单</b><span>${done} / ${todos.length} 项</span><i class="bui-chevron">${icon('chevron')}</i></summary><ol>${todos.map((t,i)=>{
    const status=t.status==='in_progress'&&state.status?.type==='idle'?'interrupted':t.status;
    const label={completed:'已完成',in_progress:'进行中',pending:'待处理',cancelled:'已取消',interrupted:'已中断'}[status]||'待处理';
...
export function requestsHTML(state){
  return (state.questions||[]).map(q=>`<form class="request-card bui-approval" data-question="${esc(q.id)}"><div class="approval-kicker">${icon('skill')}<span>需要你的补充</span></div>${q.questions.map((question,i)=>`<fieldset><legend>${esc(question.question)}</legend><div class="approval-options">${question.options.map(o=>`<label><input type="${question.multiple?'checkbox':'radio'}" name="q${i}" value="${esc(o.label)}"><span><b>${esc(o.label)}</b><small>${esc(o.description||'')}</small></span></label>`).join('')}</div><input type="text" name="free${i}" placeholder="也可以直接输入答复…" aria-label="${esc(question.question)}的补充答复"></fieldset>`).join('')}<footer><span>${q.questions.length} 个问题</span><button type="submit" class="approval-confirm">提交并继续 →</button></footer></form>`).join('')+
  (state.permissions||[]).map(p=>`<div class="request-card bui-approval"><div class="approval-kicker">${icon('shield')}<span>操作确认</span></div><h3>${esc(p.description||({bash:'运行文件处理命令',edit:'修改工作文件',external_directory:'访问工作材料',read:'读取文件'})[p.permission]||'执行工具操作')}</h3><p>允许 Agent 继续这项操作吗？</p><details class="approval-technical"><summary>技术详情</summary><pre class="approval-scope">${esc((p.patterns||[]).join('\n'))}</pre></details><footer><span title="同类允许遵循原生请求给出的匹配范围，仅在当前对话生效">按原生权限范围授权</span><button data-permission="${esc(p.id)}" data-reply="reject">拒绝</button><button data-permission="${esc(p.id)}" data-reply="always" title="同类范围：${esc((p.always||p.patterns||[]).map(x=>x==='*'?'当前对话内该类操作，仍遵守禁止规则':x).join('；'))}">本对话同类允许</button><button class="approval-confirm" data-permission="${esc(p.id)}" data-reply="once">本次允许 ${icon('check')}</button></footer></div>`).join('');
}
export function orderedDocuments(documents){return [...documents.filter(d=>!d.thread_id),...documents.filter(d=>d.thread_id)];}
export function documentTab(d,selected){return `<button class="document-tab ${selected===d.id?'active':''}" data-open-attachment="${esc(d.id)}" aria-pressed="${selected===d.id}" title="${esc(d.filename)}"><span class="file-tag">${d.thread_id?esc(d.suffix?.slice(1).toUpperCase()||'文'):'主合同'}</span><b>${esc(d.filename)}</b></button>`;}
