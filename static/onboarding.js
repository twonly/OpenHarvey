...
export async function startTour(me,{force=false}={}){
 ...
 const areas=onAgent?[
  [tr('对话区'),'#chatPane',tr('直接提问、选择审查任务，随时追问你的合同助手。')],
  [tr('合同原文'),'#docPane',tr('阅读招标文件；点击回答里的引用，可回到对应原文。')],
  [tr('产出物'),'#artPane',tr('查看已保存的摘要、审查报告和修改稿，并下载使用。')],
  [tr('工作区'),'#workspaceRail',tr('在一份合同下管理多段对话、附件和产出物。')]
 ]:[[tr('合同列表'),'.workspace-card',tr('每份主合同都有独立空间，相关对话、附件和成果集中保存。')],[tr('上传入口'),'#uploadButton',tr('从这里上传自己的主合同，自动建立一个新的合同空间。')]];
 function position(){
 ...
 function render(){
  ...
  if(step===0)content.innerHTML=ui`<p class="tour-kicker">第一步 · 看懂工作区</p><h2 id="tourTitle">一份合同，一个完整的工作现场</h2><p>从原文出发，和助手一起完成工作。</p><div class="tour-regions">${areas.map((a,i)=>`<button data-region="${i}" aria-pressed="${i===region}">${a[0]}</button>`).join('')}</div><div class="tour-region-copy"><b>${areas[region][0]}</b><p>${areas[region][2]}</p></div>`;
  if(step===1)content.innerHTML=ui`<p class="tour-kicker">第二步 · 换成你的合同</p><h2 id="tourTitle">上传，从「合同空间」开始</h2><p>点击顶部的「合同空间」，再选择「上传新合同」。每份主合同都会建立独立空间。</p><div class="tour-upload-path"><span>合同空间</span><i>→</i><span>＋ 上传新合同</span></div><p class="tour-note">支持 PDF、Word、Markdown 和 TXT。补充材料可在对话左侧的「附件」中添加。</p><button class="tour-action" data-upload>${onAgent?tr('前往合同空间'):tr('选择我的合同')} ↗</button>`;
  if(step===2)content.innerHTML=ui`<p class="tour-kicker">第三步 · ${isDemo?tr('留住你的工作成果'):tr('按你的方式工作')}</p><h2 id="tourTitle">${isDemo?tr('注册，让试用继续'):tr('连接你自己的模型')}</h2><p>${isDemo?tr('注册后，当前试用的合同与对话会保留在账号中，并获得更多试用次数。'):tr('合同、对话与成果已归属于你的账号，可以随时回来继续。')}</p><div class="tour-benefits"><span>✓ ${isDemo?tr('保留试用资料'):tr('集中保存资料')}</span><span>✓ ${isDemo?tr('更多试用次数'):tr('使用平台试用额度')}</span><span>✓ 在设置中配置自己的模型与 API Key</span></div><a class="tour-action" href="${isDemo?'/login?register=1':'/model'}">${isDemo?tr('免费注册并保留资料'):tr('前往模型设置')} ↗</a>`;
  dialog.querySelector('[data-prev]').hidden=step===0;dialog.querySelector('[data-next]').textContent=step===2?tr('开始使用 ✓'):tr('下一步 →');
