...
export function riskBoard(data,documents,filter='attention'){
  ...
  const filters=[['attention',`需关注 ${attention}`],['all',`全部 ${findings.length}`],['未命中',`未命中 ${counts['未命中']}`],['不适用',`不适用 ${counts['不适用']}`],['handled','已反馈']];
  ...
  return `<section class="risk-board" data-risk-board="${esc(data.id)}" data-risk-filter="${esc(filter)}">
    ...
    <div class="risk-list">${visible.map(f=>{
      ...
      return `<article class="review-risk" data-risk-id="${esc(f.risk_id)}" data-verdict="${esc(status)}">
        <header><span class="risk-verdict">${esc(status)}</span><h4>${esc(f.risk_name||f.risk_id)}</h4><span class="risk-severity" data-level="${esc(f.severity)}">${esc(f.severity||'未分级')}${f.severity?'风险':''}</span></header>
        <div class="risk-meta">${esc(f.category||'')} · ${esc(f.risk_id)}<span class="disposition">${esc(saved.decision)}</span></div>
        <div class="risk-reason markdown">${markdown(f.reason,documents)}</div>
        ${f.suggestion?`<div class="risk-suggestion markdown"><b>建议</b>${markdown(f.suggestion,documents)}</div>`:''}
        <details class="risk-feedback"><summary>${saved.revision?'查看 / 修改处理意见':'添加处理意见'}${saved.note?' · '+esc(saved.note.slice(0,35)):''}</summary>
          <form data-risk-feedback="${esc(f.risk_id)}" data-revision="${saved.revision}">
            <label>处理方式<select name="decision" aria-label="${esc(f.risk_name||f.risk_id)}处理方式">${decisions.map(d=>`<option${saved.decision===d?' selected':''}>${d}</option>`).join('')}</select></label>
          ...
          ${saved.revision?`<details class="feedback-history"><summary>反馈记录（${saved.history.length+1}）</summary>${[...saved.history,saved].reverse().map(h=>`<p><b>${esc(h.decision)}</b> · ${new Date(h.created*1000).toLocaleString()}<br>${esc(h.note||'无备注')}</p>`).join('')}</details>`:''}
        </details>
      </article>`;
    }).join('')||'<p class="risk-empty">这个筛选下没有审查项。</p>'}</div>
  </section>`;
}
