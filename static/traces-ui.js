...
export async function renderTraces(host,{identity,notice}){
 ...
 const stateLabel=r=>r.deleted_at?' · 已删除':r.archived_at?' · 已归档':'';
 ...
 async function loading(node,fn,force=false){
  ...
  node._loading=(async()=>{try{if(!node.dataset.loaded)body.innerHTML='<p class="settings-note">正在读取…</p>';await fn(body);node.dataset.loaded='1';}catch(e){if(node.isConnected){body.innerHTML=`<p class="trace-error-label">${esc(e.message)}</p><button data-trace-retry>重新读取</button>`;delete node.dataset.loaded;}}finally{node._loading=null;}})();
 ...
 async function threads(node,force=false){return loading(node,async body=>{
  ...
  if(!body.querySelector('.trace-session-choices'))body.innerHTML='<nav class="trace-session-choices" aria-label="切换会话"></nav><div class="trace-session-panels"></div>';
  reconcile(body.querySelector('.trace-session-choices'),data.threads,r=>{const button=document.createElement('button');button.type='button';button.className='trace-session-choice';button.dataset.key=r.id;button.dataset.traceSession=r.id;return button;},(button,r)=>{
   const html=`<b>${esc(r.title)}${stateLabel(r)}</b><span>${r.run_count} 轮 · ${esc(r.id.slice(0,8))}</span>`;
   if(button.innerHTML!==html)button.innerHTML=html;button.title=label('会话',r.id);
  });
  reconcile(body.querySelector('.trace-session-panels'),data.threads,r=>sessionPanel(r.id),(panel,r)=>summary(panel,`<b>${esc(r.title)}${stateLabel(r)}</b><span>按发生顺序 · 点击行查看详情</span>`));
  if(!data.threads.length){body.innerHTML='<p class="settings-note">暂无匹配会话。</p>';return;}
 ...
 async function turns(node,force=false,more=false){return loading(node,async body=>{
  ...
  if(!body.querySelector('.trace-turns'))body.innerHTML='<div class="trace-table-scroll"><table class="trace-turns" aria-label="会话轮次"><thead><tr><th scope="col">轮次 / 用户消息</th><th scope="col">状态</th><th scope="col">模型</th><th scope="col">耗时 / 用量</th><th scope="col">工具</th></tr></thead></table></div><button data-trace-more hidden>继续查看后续轮次</button>';
 ...
 async function list(){
  ...
  reconcile($('#traceList'),data.contracts,r=>group('trace-contract',r.id),(node,r)=>summary(node,`<div class="trace-group-label"><b>${esc(r.title)}</b><small title="${label('合同',r.id)}">合同 ${esc(r.id.slice(0,8))} · ${esc(r.username)}</small></div><span class="trace-group-count">${r.thread_count} 个会话 · ${r.run_count} 轮记录</span>`));
  if(!data.contracts.length)$('#traceList').innerHTML='<p class="settings-empty">还没有匹配的记录。</p>';
  $('#traceCount').textContent=`${data.total} 份合同`;host.querySelector('[data-trace-prev]').disabled=offset===0;host.querySelector('[data-trace-next]').disabled=offset+20>=data.total;
  for(const node of host.querySelectorAll('.trace-contract[open]'))await threads(node,true);
 }
 async function refresh(){
  if(refreshing)return;refreshing=true;const button=host.querySelector('[data-refresh-traces]'),slot=$('#traceSyncStatus');button.disabled=true;slot.textContent='正在同步原生执行记录…';
  try{const ids=$('#traceMember')?.value?[$('#traceMember').value]:members.length?members.map(m=>m.id):[identity.id];let unavailable=0;
   ...
   if(!host.isConnected)return;await list();slot.textContent=unavailable?`${unavailable} 个会话的运行环境暂不可用，已保留检索摘要。`:'';
 ...
 host.addEventListener('click',run(async e=>{
  ...
  if(b.hasAttribute('data-load-audit')){const rows=await api('/api/admin/audit');if(!host.isConnected)return;$('#traceAudit').innerHTML=rows.map(r=>`<p>${new Date(r.created*1000).toLocaleString()} · ${esc(r.username)} · ${esc(r.action)}<small>${esc(r.target)}</small></p>`).join('')||'<p>暂无管理操作记录。</p>';}
 }));
