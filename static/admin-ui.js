...
const stateLabel=r=>!r?tr('尚未开通'):r.status==='disabled'?tr('已停用'):r.status==='failed'?tr('开通失败'):r.status==='provisioning'?tr('开通中'):r.error?tr('生效失败'):r.desired_revision!==r.applied_revision?tr('待生效'):tr('可用');
const time=v=>v?new Date(v*1000).toLocaleString(getLanguage()):'';
export async function renderAdmin(host,{mode,identity,notice,dirty,leave}){
 ...
 async function operation(value){
  ...
  const slot=$('#adminOperation');slot.innerHTML=tr('<span>正在处理… 可以继续使用工作台。</span>');
  try{
   const result=await waitForOperation(id,{read:id=>api('/api/operations/'+id),active:()=>host.isConnected});
   if(result&&host.isConnected){
    pending=false;slot.innerHTML=`<span class="${result.status==='failed'?'trace-error-label':''}">${result.status==='completed'?esc(result.result.message||(result.result.users?.some(u=>!u.applied)?tr('已保存，将在当前任务结束后生效。'):tr('操作已完成'))):esc(result.result.message||tr('操作失败'))}</span><small>${time(result.updated)}</small>`;
    await loadList();if(mode==='members'&&selected){const item=items.find(m=>m.id===selected.id);const tag=$('#memberForm .settings-tag');if(item&&tag)tag.textContent=item.active?stateLabel(item.runtime):tr('已停用');}return result;
   }
 ...
 function frame(title,caption,action){
  host.innerHTML=ui`<div class="settings-heading"><div><h2>${title}</h2><p>${caption}</p></div>${action||''}</div><div id="adminOperation" class="admin-operation" role="status"></div><div class="settings-split"><div class="settings-list-panel"><input id="adminSearch" type="search" placeholder="${tr(mode==='members'?'搜索成员':'搜索供应商')}" aria-label="${tr(mode==='members'?'搜索成员':'搜索供应商')}"><div id="adminList"></div></div><div id="adminEditor"><p class="settings-empty">${tr(admin?'选择一项查看或编辑。':'选择一项查看。')}</p></div></div>${mode==='providers'&&admin?'<div id="modelApplyNotice" class="admin-operation" role="status" hidden></div>':''}`;
 ...
 async function loadList(){
  if(mode==='members'){items=await api('/api/admin/members');list();}
   ...
   if(slot){
    const waiting=[{username:tr('我的运行环境'),runtime:data.runtime}].filter(m=>m.runtime&&m.runtime.status!=='disabled'&&(m.runtime.error||m.runtime.desired_revision!==m.runtime.applied_revision));
    slot.hidden=!waiting.length;
    slot.innerHTML=waiting.map(m=>`<p><b>${esc(m.username)}</b> · ${m.runtime.error?ui`<span class="trace-error-label">配置未生效：${esc(m.runtime.error)}</span>`:tr('新配置将在当前任务结束后生效。')}</p>`).join('')+(waiting.length?tr('<button data-apply-models>重新验证并应用</button>'):'');
   }
  }
 }
 function list(){
  const q=$('#adminSearch').value.toLowerCase();
  $('#adminList').innerHTML=items.filter(x=>(mode==='members'?x.username:x.label+' '+x.id).toLowerCase().includes(q)).map(x=>`<button data-admin-item="${esc(x.id)}" class="settings-list-item ${selected?.id===x.id?'active':''}"><b>${esc(mode==='members'?x.username:x.label)}</b><span>${mode==='members'?(x.role==='admin'?tr('管理员'):tr('成员'))+' · '+(x.active?stateLabel(x.runtime):tr('已停用')):(x.enabled?tr('启用'):tr('已停用'))+' · '+x.models.length+tr(' 个模型')}</span>${mode==='members'&&x.runtime?.error?`<p>${esc(x.runtime.error)}</p>`:''}</button>`).join('')||tr('<p class="settings-empty">暂无匹配记录。</p>');
 }
 function memberEditor(item){
  selected=item;list();
  $('#adminEditor').innerHTML=ui`<form id="memberForm" class="settings-form"><div class="settings-editor-head"><h3>${item?tr('成员设置'):tr('创建成员')}</h3>${item?`<span class="settings-tag">${item.active?stateLabel(item.runtime):tr('已停用')}</span>`:''}</div>${input(tr('账号'),'username',item?.username,'required pattern="[a-z][a-z0-9_-]{1,31}" maxlength="32" '+(item?'readonly':tr('autocomplete="off" placeholder="例如 chen-li"')))}<label>角色<select name="role"><option value="member" ${item?.role!=='admin'?'selected':''}>普通成员</option><option value="admin" ${item?.role==='admin'?'selected':''}>管理员</option></select></label>${item?ui`<label class="settings-check"><input type="checkbox" name="active" ${item.active?'checked':''}>账号启用</label><p class="settings-note">停用将撤销登录并停止运行环境，合同、历史和产出物仍保留。</p>`:ui`${input(tr('初始密码'),'password','Contract1','minlength="8" maxlength="200" autocomplete="new-password" type="password"')}<p class="settings-note">创建后自动准备独立运行环境。状态变为“可用”后即可登录使用。</p>`}<div class="settings-form-actions"><button type="submit" class="primary">${item?tr('保存成员设置'):tr('创建并开通')}</button>${item?.active?tr('<button type="button" data-retry-member>重新开通 / 恢复运行</button>'):''}</div>${item?ui`<details><summary>重置密码</summary>${input(tr('新的登录密码'),'reset_password','Contract1','type="password" minlength="8" autocomplete="new-password"')}<button type="button" data-reset-password>重置密码并撤销登录</button></details>`:''}</form>`;
 ...
 function renderCatalog(){
  ...
  root.innerHTML=ui`<div class="provider-catalog-head">${catalog.length} 个模型，点击添加到配置</div><input type="search" id="modelFilter" aria-label="筛选模型列表" placeholder="搜索模型 ID"><div class="provider-catalog-items"></div><p class="settings-note">列表来自当前服务；加入后仍需保存并应用。模型上限可在高级参数中调整。</p>`;
 ...
 function catalogList(){
  ...
  $('#modelCatalog .provider-catalog-items').innerHTML=catalog.filter(id=>id.toLowerCase().includes(q)).map(id=>`<button type="button" data-catalog-model="${esc(id)}" ${ids.has(id)?'disabled':''}>${esc(id)}${ids.has(id)?tr('<small>已添加</small>'):''}</button>`).join('')||tr('<p class="settings-empty">暂无匹配模型。</p>');
 }
 async function fetchModels(button){
  ...
  button.disabled=true;button.textContent=tr('获取中…');resultText(slot,tr('正在获取模型列表…'));$('#modelCatalog').hidden=true;
  try{
   const data=await api('/api/providers/models',{method:'POST',body});
   if(!form.isConnected)return;
   if(JSON.stringify(connectionBody())!==stamp){resultText(slot,tr('接入配置已修改，请重新获取模型列表。'));return;}
 ...
 async function testModel(button){
  ...
  const revision=form.dataset.editRevision||'0';button.disabled=true;button.textContent=tr('测试中…');resultText(slot,tr('正在测试当前模型…'));
  try{
   ...
   if((form.dataset.editRevision||'0')!==revision){resultText(slot,tr('配置已修改，请重新测试。'));return;}
   if(result.status==='failed'){resultText(slot,result.result.message||tr('连接测试失败'),true);return;}
   const ms=result.result.models?.[0]?.latency_ms;
   resultText(slot,tr('连接成功')+(ms!=null?' · '+(ms/1000).toFixed(2)+tr(' 秒'):'')+' · '+model.id);
  }catch(e){if(row.isConnected)resultText(slot,e.message,true);}
  finally{button.disabled=false;button.textContent=tr('测试连接');}
 }
 ...
 host.addEventListener('click',run(async e=>{const b=e.target.closest('button');if(!b)return;
  ...
  if(b.dataset.catalogModel){collectModels();if(currentModels.length>=40&&!currentModels.some(m=>!m.id.trim()))throw Error(tr('每个供应商最多配置 40 个模型'));currentModels=addDiscoveredModel(currentModels,b.dataset.catalogModel);modelForms();catalogList();dirty(true);}
