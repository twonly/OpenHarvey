...
export async function setupAuth(){
 ...
 layout.innerHTML=`<section class="auth-story"><a class="entry-brand" href="/"><img src="/static/brand/agent-logo-v1.png" width="34" height="34" alt="">合同工作台</a><div class="auth-story-copy"><p class="auth-eyebrow">LESS PAPERWORK. MORE CLARITY.</p><h1>每一份合同，<br>都有一个好帮手。</h1><p>从读懂条款到完成审查，<br>让原文、对话和成果，在同一处连接。</p></div><div class="auth-workspace-preview" aria-label="工作区功能示意"><div class="preview-top"><span class="preview-dot"></span>虚构采购服务合同<span>虚构示例 · TXT</span></div><div class="preview-body"><div class="preview-paper"><span>招标文件</span><b>项目需求与技术要求</b><i></i><i></i><i class="marked"></i><i></i><small>原文 · 可定位引用</small></div><div class="preview-conversation"><span class="preview-question">帮我梳理这份文件的重点</span><div class="preview-reply"><img src="/static/brand/agent-logo-v1.png" width="23" height="23" alt=""><div><b>从原文出发，逐条理清</b><span>项目范围 / 商务要求 / 履约安排</span><small>对话与原文，随时对照</small></div></div><div class="preview-output">↗ <span>摘要、报告、修改稿</span><small>统一归档</small></div></div></div></div><div class="auth-story-footer"><span>01 阅读原文</span><span>02 对话协作</span><span>03 保存成果</span></div></section><section class="auth-panel"><a class="auth-back" href="/">← 返回首页</a><div class="auth-form-host"></div><p class="auth-footnote">你的合同、对话与模型配置，集中在自己的空间。</p></section>`;
...
export async function accountReady(me){
...
export function orcaConnect(host,identity){
 ...
 if(identity.account_kind==='demo')panel.innerHTML=`<h2>使用自己的模型</h2><p>demo 用户无法配置模型。注册后可保留资料，并连接自己的 API Key。</p><a class="primary" href="${registerURL}">注册并继续</a>`;
...
document.addEventListener('registration-required',event=>{if(document.getElementById('registrationPrompt'))return;const d=document.createElement('dialog');d.id='registrationPrompt';d.className='account-onboarding';d.innerHTML=`<h2>继续使用合同助手</h2><p>${esc(event.detail)}</p><a href="${document.body.dataset.accountKind==='demo'?registerURL:'/model'}">${document.body.dataset.accountKind==='demo'?'注册并保留资料':'配置自己的模型'}</a> <button>稍后</button>`;document.body.append(d);d.querySelector('button').onclick=()=>d.close();d.onclose=()=>d.remove();d.showModal();});
