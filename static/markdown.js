...
export function citations(escaped,documents=[],allowBare=true){
  ...
  return escaped.replace(/【(?:D([a-f0-9]{12}):)?B(\d+)(?:-B(\d+))?】/g,(_,id,block,end=block)=>{
    if(!id){
      if(!allowBare||documents.length!==1)return '<span class="invalid-citation" title="引用缺少来源文件，无法唯一定位">来源待核对</span>';
      id=documents[0].id;
      if(!documents[0].locations)return '<span class="invalid-citation" title="缺少段落定位数据">来源待核对</span>';
    }
    const doc=valid.get(id),location=doc?.locations?.['B'+block],last=doc?.locations?.['B'+end];
    const start=Number(block),stop=Number(end);
    if(!doc||!Number.isSafeInteger(start)||!Number.isSafeInteger(stop)||stop<start||stop-start>100000||(doc.locations&&(!location||!last||!Array.from({length:stop-start+1},(_,i)=>doc.locations['B'+(start+i)]).every(Boolean))))return '<span class="invalid-citation" title="材料已移除或引用位置不存在">引用暂不可用</span>';
    const a=location?.ordinal||start+1,b=last?.ordinal||stop+1;
    const where=location?.page&&last?.page?(location.page===last.page?`第 ${location.page} 页`:`第 ${location.page}–${last.page} 页`):(a===b?`第 ${a} 段`:`第 ${a}–${b} 段`);
    const name=doc.thread_id?(doc.filename||'附件'):'查看原文';
    const position=(location?.page?'PDF · ':'')+where;
    const preview=location?.preview?'\n'+location.preview:'';
    return `<button class="citation" data-doc="${id}" data-block="B${block}" data-end="B${end}" data-hash="${esc(doc.source_hash)}" title="${esc(doc.filename||name)} · ${position}${stop>start?' · 连续 '+(stop-start+1)+' 段':''}，点击查看原文${esc(preview)}"><span class="citation-name">${esc(name)}</span><span aria-hidden="true"> · </span><span>${position}</span></button>`;
  });
}
