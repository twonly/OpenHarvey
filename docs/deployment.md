...
`PYTHONPATH=. python scripts/export_website.py /absolute/new/output-directory` generates a static Chinese homepage, `/en`, bilingual information pages, sitemap and crawler files. It does not include business data. The hosted website links to the existing live demo; customize application URLs for your own instance.
